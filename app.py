...
def index():
        ...
        if city:
            ...
            if lat is not None and lon is not None:
                try:
                    ...
                    forecast_hourly_url = point_resp.json()["properties"]["forecastHourly"]
                    hourly_resp = requests.get(forecast_hourly_url)
                    ...
                    hourly_weather = generate_hourly_weather_report(hourly_data)
                except Exception as e:
                    error = str(e)
            else:
    ...
    return render_template("index.html", weather=weather, weekly_weather=weekly_weather, hourly_weather=hourly_weather, error=error, city=city if request.method == "POST" else None)


# hourly weather report
def generate_hourly_weather_report(forecast_data):
    hourly_report = []
    ...
    end_time = curr_time + timedelta(hours=24)
    ...
    for period in periods:
        ...
        if curr_time <= start_time < end_time:
            ...
            hourly_report.append(hourly_summary)
        ...
        for period in day_data["day_periods"]:
            if "temperature" in period:
                ...
                if period.get("isDaytime", True):  # Daytime 
                    ...
                    if day_summary["daily_short_forecast"] is None:
                        day_summary["daily_short_forecast"] = period["shortForecast"]
                        day_summary["daily_icon"] = period["icon"]
